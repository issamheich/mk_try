  ...
  Future<void> fetchData() async {
    try {
      ...
      final popularResponse = await http.get(Uri.parse(
          'https://api.themoviedb.org/3/tv/popular?api_key=8f78d38fb24129522724f8f66f81c75c&page=10'));
      ...
      if (movieResponse.statusCode == 200 &&
        ...
        setState(() {
          movies = List<Movie>.from(
            movieData['results'].map((json) {
              try {
                return Movie.fromJson(json, genreMapping);
              } catch (e) {
          ...
          tv = List<TVShow>.from(
            tvData['results'].map((json) {
              try {
                return TVShow.fromJson(json, genreMapping);
              } catch (e) {
                print('Error parsing TV show: $e\n$json');
                return null;
              }
            }).where((tvShow) => tvShow != null),
          );

          popular = List<TVShow>.from(
            popularData['results'].map((json) {
              try {
                return TVShow.fromJson(json, genreMapping);
              } catch (e) {
                print('Error parsing popular TV show: $e\n$json');
                return null;
              }
            }).where((tvShow) => tvShow != null),
          );
        ...
        print(
            'Failed to load data: ${movieResponse.statusCode}, ${tvResponse.statusCode}, ${popularResponse.statusCode}');
                ...
                SliverList(
                  delegate: SliverChildListDelegate([
                    ...
                    MediaSection(
                      ...
                      media: popular,
                      addToList: (media) => addOrRemoveFromList(context, media),
                      onTap: (media) {},
                    ),
                    MediaSection(
                      title: "Top TV Shows",
